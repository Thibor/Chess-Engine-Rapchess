...
var maskEE = colorEnemy | colorEmpty;
var his = [];
...
if(add){
	var p = g_board[to] & 7;
	var m = fr | (to << 8) | flags;
	var l = moveStack.length;
	if(p)
		moveStack[l] = m;
	else{
		if(l > countNA)
			moveStack[l] = moveStack[countNA];
		moveStack[countNA++] = m;
	}
...
function GetScore(mu,depth,depthL,depthQ,alpha,beta,score){
var n = mu.length;
var noCheck = n;
var alphaDe = 0;
var alphaFm = '';
...
while(n--){
	...
	if(g_inCheck){
		...
		osScore = 0;
	else if(depth < depthQ){
			var os = GetScore(me,depth + 1,depthL,depthQ,-beta,-alpha,-osScore);
			osScore = -os.score;
			osPv = ' ' + os.pv;
			osDe = os.depth;
	}
	...
	if((!g_stop)&&(alpha < osScore)){
		...
		if(depth == 1){
			...
			postMessage('info currmove ' + bsFm + ' currmovenumber ' + n + ' nodes ' + g_totalNodes + ' nps ' + nps + ' depth ' + g_depth + ' seldepth ' + alphaDe + ' score ' + g_scoreFm + ' pv ' + bsPv);
		}
	}
	if(alpha >= beta)break;
}
...
do{
	...
	mu.splice(bsIn,1);
	g_depth++;
	if(os.score < - 0xf000 || os.score > 0xf000)break;
}while((!depth || (g_depth < depth)) && !g_stop && m1);
var nps = Math.floor((g_totalNodes / ((new Date()).getTime() - g_startTime)) * 1000);
